...
| **Magic link** | Technique | Lien de connexion à usage unique et à durée limitée, envoyé par email. C'est l'authentification du restaurateur puis des membres nominatifs : pas de mot de passe. Le client, lui, n'a jamais de compte. | [Sécurité](architecture/securite.md) |
| **Membre d'équipe** | Métier | Personne invitée à travailler sur un établissement avec un rôle `owner`, `manager`, `service` ou `kitchen`. Son rôle appartient à cet établissement et ses actions sensibles sont attribuables. | [ADR-0031](decisions/adr-0031-equipes-roles-vues-metier.md) |
...
| **Poste partagé** | Métier | Appareil Salle ou Cuisine appairé à un établissement, expirant et révocable. Il ne partage pas le magic link du restaurateur et n'accède jamais aux finances ni à l'administration. | [ADR-0031](decisions/adr-0031-equipes-roles-vues-metier.md) |
...
| **QR code** | Technique | Code-barres bidimensionnel imprimé à table ou sur le comptoir. Le client le scanne pour ouvrir le mini-site de l'établissement et commander. Il enrichit le support physique, sans devenir l'unique accès aux affichages obligatoires. | [Parcours de commande](produit/parcours/commande-client.md) |
| **Rapprochement Surplasse** | Métier | Explication des ventes du canal Surplasse par ses commandes, paiements, remboursements, commissions, frais disponibles et versements Stripe. Il ne représente pas le chiffre d'affaires total du restaurant. | [Socle professionnel](produit/socle-professionnel.md) |
...
| **Socle professionnel** | Métier | Ensemble minimal de fonctionnalités qui permet à une équipe cible d'exploiter Surplasse chaque jour sans contournement bloquant : rôles, vues métier, gestion sûre de la carte, exceptions et rapprochement. | [Socle professionnel](produit/socle-professionnel.md) |
...
| **Vue métier** | Métier | Présentation du Dashboard adaptée au travail en Salle, Cuisine ou Gestion. Elle organise l'information mais ne porte aucun droit, toujours vérifié par le Backend. | [Parcours Dashboard](produit/parcours/dashboard-restaurateur.md) |
...
| « sous-compte », « employé » comme terme générique | Membre d'équipe | Les intitulés de poste réels peuvent rester affichés dans le profil |
| « application serveur », « application cuisine » | Vue métier Salle ou Cuisine | Le Dashboard reste une seule application |
| « compte commun » | Poste partagé | Aucun partage de magic link |
