...
Le canal par établissement est authentifié par la session restaurateur, portée par le cookie `HttpOnly` de portée `.surplasse.com` (voir [la sécurité](securite.md)). Ce point est structurant : l'API navigateur `EventSource` n'accepte aucun en-tête personnalisé, donc aucun `Authorization: Bearer` ; c'est le cookie de session, envoyé automatiquement en même site, qui authentifie le flux. Le canal par commande est adressé par un jeton non devinable propre à la commande : le client n'a pas de compte, le jeton fait office de capacité d'accès (transmis en paramètre de l'URL du flux).
