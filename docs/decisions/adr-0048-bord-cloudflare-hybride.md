...
Le 2026-08-25, l'origine Atlas répondait 525 depuis Cloudflare. Une activation opérateur d'urgence a donc attaché la version vérifiée du Worker aux seules Routes de l'apex et de `www` afin de restaurer l'Onboarding. Cette exception d'incident ne change pas la propriété du plan de contrôle. `vps-infra` doit importer cet état exact avant toute extension vers un hôte statique, l'API ou le wildcard.
