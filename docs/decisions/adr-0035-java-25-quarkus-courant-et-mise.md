...
Cette décision ne change pas l'orchestration applicative. Docker Compose reste le cycle de vie du développement intégré et du coeur Atlas, conformément aux décisions qui encadrent ces environnements. Depuis l'[ADR-0048](adr-0048-bord-cloudflare-hybride.md), Cloudflare exécute séparément le Worker et les Static Assets. mise ne démarre ni PostgreSQL, ni Caddy, ni le Backend, ni les frontends comme services. Il ne remplace pas Docker, Wrangler, dnsmasq, mkcert ou le magasin de certificats du système. Il est absent du VPS de production.
