...
Le wrapper matérialise atomiquement les valeurs sensibles du profil protégé dans un répertoire hôte de mode `0700`, hors de git. Les copies destinées au montage utilisent le mode `0444` : le répertoire interdit leur lecture aux autres utilisateurs de l'hôte, tandis que le mode du fichier permet aux différents UID non privilégiés des conteneurs de le lire. Compose monte ensuite ces sources en lecture seule sous `/run/secrets`. Le fichier d'environnement de production et les clés privées TLS locale et JWT de production d'origine restent en mode `0600`. Cette étape utilise exclusivement la source `file`, compatible avec les services en lecture seule sur les moteurs supportés. PostgreSQL et Grafana consomment leurs interfaces `*_FILE` natives. Les points d'entrée du Backend, de l'Onboarding de développement et de Caddy lisent les fichiers nécessaires, refusent la présence simultanée d'une valeur directe et d'un fichier, puis lancent le processus. Le wrapper copie aussi les fichiers privés déjà protégés vers ces sources de montage.
