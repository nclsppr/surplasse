...
Le socle de la porte 2 suit maintenant l'[ADR-0045](decisions/adr-0045-atlas-unique-production.md). `compose.yaml` et `compose.development.yaml` assemblent uniquement le cluster local. La production consomme le fragment `deployment/vps/compose.yaml` lié dans une `application-release` immuable, puis admis et activé par `vps-infra`. La construction durcie des images, leur scan, leur publication GHCR par SHA, leur SBOM et leurs attestations sont livrés selon l'[ADR-0037](decisions/adr-0037-images-conteneurs-durcies.md). `main` produit un candidat sans autorité d'activation. L'[ADR-0041](decisions/adr-0041-production-testeurs-stripe-test.md) autorise une production testeurs avec commandes persistées, Stripe test et sauvegardes locales au VPS. Cette exception ne franchit pas la porte publique. Avant le public, il reste absolument à prouver la sauvegarde hors site restaurée, le SMTP transactionnel, Stripe live, les CSP, les sondes publiques et leur canal d'alerte, ainsi que les portes transactionnelles de cutover, reprise au démarrage, budgets et réconciliation détaillées dans le [runbook](operations/deploiement-compose.md#état-opérationnel-atlas-au-2026-08-18).
