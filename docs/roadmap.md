...
Le Dashboard minimal de cette phase est livré localement, y compris l'indicateur permanent de connexion temps réel. Le remboursement reste volontairement hors de la route de statut : il doit passer par le domaine paiement et déclencher une vraie opération Stripe. Le premier durcissement navigateur est livré : Quarkus refuse toujours les credentials CORS et Caddy les accorde seulement aux origines exactes du Dashboard et de l'Onboarding. Le découpage est couvert côté Backend et au travers d'un vrai proxy Caddy. Viennent ensuite la coordination du refresh token entre onglets, le passage Stripe en conditions live, le service à blanc, les corrections issues du pilote et enfin le service réel qui constitue le critère de sortie de la phase.

Avant toute mise en production du Dashboard, le second durcissement navigateur reste obligatoire : coordonner la rotation du refresh token entre plusieurs onglets. Le futur Caddy de production devra aussi reprendre la règle CORS déjà fermée par défaut et exercée localement. Les critères et les pistes sont détaillés dans la page [sécurité](architecture/securite.md#durcissements-dashboard-avant-production).
