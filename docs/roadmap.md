...
- CI de tests : le Backend et les frontends sont testés à chaque push et sur les pull requests Renovate, la génération est vérifiée contre le contrat.
...
Le socle de la porte 2 suit maintenant l'[ADR-0048](decisions/adr-0048-bord-cloudflare-hybride.md). `compose.yaml` et `compose.development.yaml` assemblent uniquement le cluster local. Cloudflare porte le candidat du bord et des statiques ; Atlas conserve Quarkus et PostgreSQL dans une `application-release` immuable. `vps-infra` admet et active séparément leurs références exactes. La construction durcie des images, leur scan, leur publication GHCR par SHA, leur SBOM et leurs attestations restent livrés selon l'[ADR-0037](decisions/adr-0037-images-conteneurs-durcies.md). `main` produit des candidats sans autorité d'activation. L'[ADR-0041](decisions/adr-0041-production-testeurs-stripe-test.md) autorise une production testeurs avec commandes persistées, Stripe test et sauvegardes locales au VPS. Cette exception ne franchit pas la porte publique. Avant le public, il reste absolument à prouver la sauvegarde hors site restaurée, le SMTP transactionnel, Stripe live, les CSP dans un navigateur réel, les sondes publiques et leur canal d'alerte, ainsi que les portes transactionnelles de bascule et de reprise détaillées dans les runbooks [Cloudflare](operations/migration-cloudflare.md) et [Atlas](operations/deploiement-compose.md).

Le cycle de développement est également simplifié et versionné. `mise.toml` et `mise.lock` remplacent nvm et SDKMAN pour Node, Java et Python sur les postes supportés, sans devenir un orchestrateur de services. Docker Compose reste le moteur du cluster local. En production, Atlas exécute le fragment applicatif et son cycle de vie via `vps-infra`. L'App GitHub Mend Renovate hébergée propose les mises à jour le lundi entre 0 h et 5 h, avec trois pull requests simultanées au maximum, aucune fusion automatique et une approbation préalable pour les versions majeures, la plateforme et OpenAPI Generator. Les alertes de vulnérabilité GitHub ignorent la fenêtre et les quotas pour proposer immédiatement une correction, tout en conservant la CI et la fusion manuelle. Ses PR exécutent la CI sans déployer. Une évolution de runtime impose la régénération manuelle de `mise.lock`, car le service hébergé ne peut pas exécuter `mise lock`. Ni `mise`, ni Renovate ne sont installés sur le VPS.
