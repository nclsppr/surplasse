...
Le flux client local couvre déjà le scan du QR code, la carte, le panier, la création de commande, le paiement en environnement de test et le suivi. L'identité restaurateur couvre le magic link, les cookies de session, la rotation du refresh token et l'autorisation par établissement. Le Backend expose la file paginée des commandes opérationnelles et leur avancement authentifié. Un premier Dashboard consomme cette lecture REST après une connexion par magic link, permet de sélectionner un établissement autorisé et fait avancer chaque commande de `paid` à `served` ou `picked_up` selon son type.

Les transitions opérationnelles côté restaurateur sont livrées. Le remboursement reste volontairement hors de cette route : il doit passer par le domaine paiement et déclencher une vraie opération Stripe. Il reste à livrer le flux SSE par établissement pour atteindre le Dashboard minimal de cette phase. Viennent ensuite le passage Stripe en conditions live, le service à blanc, les corrections issues du pilote et enfin le service réel qui constitue le critère de sortie de la phase.
