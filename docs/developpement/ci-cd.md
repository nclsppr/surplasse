...
La clé Stripe publiable de Commande vient de la variable de dépôt `VITE_STRIPE_PUBLISHABLE_KEY`. Sur un push vers `main`, le workflow exige une valeur au format `pk_live_`, refuse les valeurs absentes, de test ou contenant un espace, puis fige son SHA-256 pour empêcher une substitution entre les matrices. Il suit le script JavaScript chargé par `index.html`, exige que la valeur exacte y soit la seule clé live et refuse toute clé test, dans l'image scannée puis dans le digest publié. Ce contrôle ne prouve pas l'existence de la clé chez Stripe ni son appartenance au même compte que la clé secrète Backend. Ces preuves restent dans le préflight d'activation. Le Dashboard ne reçoit pas cette variable. Une pull request ne reçoit pas cette configuration de production et peut seulement construire et scanner un candidat non publiable. La clé publiable est publique et sa présence dans les journaux, les arguments et la provenance de construction est assumée. Aucun secret Stripe, SMTP, PostgreSQL, JWT ou DNS n'entre dans le workflow ou dans un argument de build.
