...
!!! info État actuel
Depuis le 2026-07-18 (phase 1) existent : la documentation (`docs/`), la charte graphique (`brand/`), la landing statique de l'Onboarding, le contrat (`api/openapi.yaml`) avec son lint et sa chaîne de génération, et le squelette du Backend (`backend/` : modules `common`, `contract`, `catalog`, `application`, migrations Flyway et seed de démonstration). Les frontends applicatifs arrivent au fil de la phase 1 ; cette page passe du statut de cible au réel au fur et à mesure. Cette page décrit la cible de référence, au présent de spécification, et elle est tenue à jour au fil des phases de la [roadmap](../roadmap.md) : à chaque application créée, la section correspondante passe de la cible au réel.
!!!
...
**4. Ouvrir l'établissement de démonstration.** En production, Commande résout l'établissement depuis le sous-domaine (`{slug}.surplasse.com`). En développement, le slug est fourni autrement (variable d'environnement ou segment d'URL : le mécanisme exact reste à trancher et sera fixé dans [conventions React](conventions-react.md)). Ouvrir le mini-site de l'établissement de démonstration dans le navigateur : la carte s'affiche, on peut composer un panier et, si les clés Stripe de test sont renseignées, dérouler un paiement fictif de bout en bout avec les cartes de test Stripe (`4242 4242 4242 4242`).
