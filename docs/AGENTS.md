...
Les domaines sont des données de configuration. `config/domains/production.env` et `config/domains/development.env` portent les valeurs publiques canoniques. La logique applicative ne code jamais le suffixe `.com` ou `.test`. Les sous-domaines `www`, `api`, `dashboard`, `docs`, `app`, `admin`, `local`, `mail`, `autoconfig`, `autodiscover`, `mta-sts`, `smtp`, `imap`, `pop`, `pop3`, `webmail`, `status`, `reports` et `grafana` sont réservés et ne peuvent pas devenir un `slug` d'établissement. `app` et `admin` restent réservés sans désigner une application actuelle. Les noms techniques réservés sans service public répondent 503 sur le bord HTTPS et ne sont jamais transmis à Commande. `REPORTS_URL` et `GRAFANA_URL` sont dérivées uniquement pour le profil development. Elles désignent respectivement le dernier rapport Allure local et l'interface Grafana locale. Les cookies restaurateur sont toujours hôte uniquement sur l'API : `COOKIE_DOMAIN` reste vide, jamais `.surplasse.com` ni `.surplasse.test`.
...
├── compose.yaml             # Graphe de services commun
├── compose.*.yaml           # Surcharges development et production
├── frontends/
...
Tout nouveau service du profil Compose development est ajouté au registre du cockpit dans le même commit avec son identifiant Compose, son URL éventuelle, sa sonde et sa politique de contrôle. Le cockpit appelle seulement le wrapper `scripts/compose.sh development` avec une liste fixe de services et d'opérations. Un composant de bord dont l'arrêt couperait le cockpit, comme Caddy, reste visible en lecture seule. Un processus natif, un module bibliothèque ou un outil interactif qui ne peut pas être piloté de façon sûre est documenté comme manuel, jamais transformé en commande arbitraire depuis le navigateur.
