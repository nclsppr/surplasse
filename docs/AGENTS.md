...
Tout nouveau service du profil Compose development expose dans le même commit une commande npm ou Compose bornée, son URL éventuelle, sa sonde, son mode d'arrêt et sa politique de données. Un processus natif, un module bibliothèque ou un outil interactif est documenté avec sa commande directe. Aucun navigateur local ne devient un orchestrateur de commandes.
...
Le déploiement Pages est automatique : chaque push sur `main` publie la documentation Nimbus sous `/docs/`, la landing statique, le tunnel avec aperçu du Dashboard, les assets de marque et le dernier rapport Allure development via `.github/workflows/pages.yml`. Le workflow actualise aussi ce rapport chaque heure sous `/local-tests/`. L'image `docs` sert le même rendu à la racine de `DOCS_URL` dans le profil local et dans la release Atlas.
